    ...
    def compute(self, node, input_vals):
        ...
        if node.matmul_attr_trans_A :
            if node.matmul_attr_trans_B :
                return np.dot(input_vals[1], input_vals[0]).T
            else:
...
def gradients(output_node, node_list):
    ...
    for node in reverse_topo_order :
        ...
        in_grads = node.op.gradient(node, node_to_output_grad[node])
        for i, in_node in enumerate(node.inputs):
            if in_node in node_to_output_grads_list:
                node_to_output_grads_list[in_node].append(in_grads[i])
            else:
                node_to_output_grads_list[in_node] = [in_grads[i]]
